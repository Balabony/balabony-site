...
Правила:
1. continuity — лише РЕАЛЬНІ суперечності між ЦИМ епізодом і попередньою історією чи каноном:
   - події: персонаж, що раніше поїхав / помер / посварився / був кудись прийнятий, тут діє так, ніби цього не було;
   - імена та клички: інше ім'я чи по-батькові персонажа, інша кличка тварини, плутанина двох різних осіб, новий персонаж на роль, яку в каноні вже займає інший (напр. інший баяніст / поштар / тракторист);
   - факти й родинні зв'язки, що суперечать встановленим.
   Для кожної вкажи source — епізод-джерело (напр. S2E14) або "канон".
2. severity: "error" — пряма суперечність факту; "warn" — підозра / нечітко.
3. voices — лише явні збої характеру чи тону персонажа (говорить геть не у своєму характері, чужа катчфраза).
4. НЕ вигадуй проблем. Якщо все узгоджено — повертай порожні масиви й коротке summary.
5. ВАЖЛИВО — НЕ повідомляй про таке (це перевіряє окремий механічний інструмент, не дублюй його роботу):
   - зачини-штампи («Усе почалося з того…» тощо);
   - формат фіналу (авторська нарація проти репліки);
   - міські формули в селі (під'їзд, поверх, квартира тощо);
   - латиницю, орфографію, правопис, довжину чи якість сюжету.
   Твоя зона — ВИКЛЮЧНО узгодженість подій, імен, фактів і характерів між епізодами.`
