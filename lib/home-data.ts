...
/**
 * Крок ротації вітрини — 1 година (рішення Богдана 16.09.2026; було 12 годин
 * з 08.09.2026, потім 3 години того ж дня).
 *
 * РАЗОМ ІЗ КРОКОМ ЗМЕНШЕНО КЕШ: `revalidate` у app/page.tsx і `s-maxage` у
 * /api/stories теж 3600. Ці три числа мусять мінятися ЗАВЖДИ РАЗОМ — кеш не
 * може бути довшим за крок, інакше читач отримає з кешу стару вітрину і
 * ротації ніби й немає.
 *
 * Нижче години не йти. Причина не технічна: читач, який повернувся на сайт
 * через годину, вже не знаходить історію, яку щойно бачив, — і це дратує
 * сильніше, ніж допомагає частіший показ. Ціна теж є: головна перебудовується
 * 24 рази на добу замість восьми, і кожна перебудова це запити в Supabase.
 */
 ...
 *
 * ПЕРЕРОБЛЕНО 16.09.2026. Було: з усього масиву лишався ПО ОДНОМУ твору на
 * автора — найсвіжіший, — і далі крутився тільки цей зріз. Порахували на
 * живих числах (978 історій, 107 авторів): на головну могли потрапити 107
 * творів, решта 871 не з'являлася ніколи, хоч як часто крути. Стеля запиту
 * у 500 рядків відрізала ще й усе старіше.
 *
 * Стало: крутяться ВСІ твори всіх авторів (рішення Богдана).
 *
 * ЯК. Вікно рухається по АВТОРАХ, а не по творах: на кожен відрізок часу
 * беремо кілька авторів поспіль по колу, і від кожного — один твір. Номер
 * цього твору залежить від того, котрий це вже оберт по всьому колу авторів.
 * Тобто автор з'являється на вітрині з тією самою частотою, що й будь-який
 * інший, але щоразу з наступним своїм текстом.
 *
 * ЧОМУ НЕ ПРОСТО ВІКНО ПО ВСІХ ТВОРАХ. Тоді автор із шістдесятьма творами
 * зайняв би вітрину на кілька діб поспіль. Правило «один автор на екран»
 * лишається, і саме воно задає швидкість: при 107 авторах і шести картках
 * повне коло авторів — 18 відрізків, тобто 18 годин.
 *
 * ЩО ЦЕ ДАЄ, перевірено симуляцією на розподілі, близькому до нашого:
 * за десять діб показується близько 92% усього каталогу; хвіст
 * найплодовитішого автора (62 твори) розтягується довше — він фізично може
 * показати лише один свій текст за кожні 18 годин, тобто на всі шістдесят
 * два йому потрібно півтора місяця. Це чесна ціна за те, що вітрина не
 * перетворюється на сторінку одного автора.
 *
 * Випадковості немає ніде: усе рахується від номера годинного відрізка,
 * тож сервер і /api/stories завжди дають однаковий набір.
 */
export function rotateDaily<T extends Row>(rows: T[], limit: number): T[] {
  if (rows.length === 0 || limit <= 0) return []

  // Автор → його твори. Ключ нормалізований: у базі трапляється різний
  // регістр і зайві пробіли. Твір без автора дістає ключ зі свого id, тож
  // такі твори поводяться як окремі «автори» і не злипаються в один.
  const byAuthor = new Map<string, T[]>()
  for (const r of rows) {
    const key = (r.author_name ?? '').trim().toLowerCase() || `id:${r.id}`
    const list = byAuthor.get(key)
    if (list) list.push(r)
    else byAuthor.set(key, [r])
  }

  // Свіжіше вперед, id як запасний ключ. approved_at буває null, тому
  // порівнюємо рядками — Date на null дав би NaN і порядок поплив би.
  const cmp = (a: T, b: T) => {
  ...
  for (const list of byAuthor.values()) list.sort(cmp)

  // Порядок авторів — за їхнім найсвіжішим твором. Стабільний: той самий
  // вхід завжди дає той самий вихід, інакше набір стрибав би між запитами.
  const authors = [...byAuthor.values()].sort((a, b) => cmp(a[0], b[0]))
  const n = authors.length

  const slot = Math.floor(Date.now() / ROTATION_STEP_MS)
  const start = ((slot * limit) % n + n) % n
  // Скільки повних кіл по авторах уже пройдено — це і є номер твору,
  // який показуємо від кожного автора цього разу.
  const round = Math.floor((slot * limit) / n)

  const out: T[] = []
  for (let i = 0; i < limit && i < n; i++) {
    const idx = (start + i) % n
    const list = authors[idx]
    // + idx: щоб усі автори не перемикали свій твір одночасно.
    out.push(list[(round + idx) % list.length])
  }
  return out
}
...
export async function getFreshStories(limit = 6): Promise<HomeStory[]> {
  try {
    ...
    const { data: light, error: lightErr } = await supabase
      ...
      // Було 500. Стеля відрізала все, що старіше, і ті твори не доходили до
      // ротації взагалі. Запит легкий — три поля, — тож беремо весь каталог.
      .limit(3000)
...
export async function getFairytales(limit = 3): Promise<HomeStory[]> {
  try {
    ...
    // Ротація на тому самому кроці, що й «Свіжі історії» — з 16.09.2026 це
    // щогодини. До 10.09.2026 рубрика показувала
    // просто три найновіші казки й не змінювалася тижнями. Порядок дій той
    // самий — спершу легкий запит із полями, потрібними rotateDaily, потім
    // повні дані лише для обраних трьох.
    const { data: light, error: lightErr } = await supabase
      .from('content')
      .select('id, author_name, approved_at')
      .eq('type', 'story')
