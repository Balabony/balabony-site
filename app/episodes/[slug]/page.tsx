...
export default async function EpisodePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Правило дослівно те саме, що в EpisodePaywall: вітрина сезону вільна,
  // преміальна замкнена завжди, адмін бачить усе.
  //
  // Вітрина: перші ТРИ серії сезону відкриті всім без вибору. Плюс одна за
  // вибором через /api/pick. Разом чотири безкоштовні серії на сезон.
  //
  // Було 1 (рішення переглянуто 12.09.2026). Однієї серії замало, щоб
  // кліфгенгер узагалі спрацював: після першої людина ще не вклалася в
  // героїв, і обрив читається як «шкода, піду далі». Після третьої вклалася
  // — і саме тоді замок стає причиною заплатити, а не причиною піти.
  //
  // Число живе у ДВОХ місцях: тут (сервер вирішує, що надіслати в браузер)
  // і в EpisodePaywall (клієнт вирішує, що показати). Міняти обидва разом —
  // інакше серія відкриється в одному розрахунку й закриється в другому.
