...
export default function SeriesListPage() {
  ...
  // Батч-генерація гачків «ДАЛІ БУДЕ» (next_teaser).
  ...
  // Батч-генерація гачків «ДАЛІ БУДЕ» (next_teaser): той самий патерн.
  // На відміну від гачка на картці, цей веде в НАСТУПНУ серію, тому сервер
  // дає моделі кінець поточної серії й початок наступної.
  const runNextTeaserBatch = async () => {
    if (ntRunning) return
    if (!confirm('Згенерувати гачки «Далі буде» для всіх серій без гачка? Уже наявні не змінюються. Це може зайняти кілька хвилин.')) return
    setNtRunning(true); setNtDone(0); setNtTotal(0); setNtLast(''); setNtText(''); setNtMsg('')
    let safety = 0
    try {
      while (safety < 500) {
        safety++
        const res = await fetch('/api/admin/next-teaser-batch', { method: 'POST' })
        const data = await res.json() as {
          done?: boolean; total?: number; remaining?: number
          processed?: { title?: string; season?: number; episode?: number; teaser?: string } | null
          error?: string
        }
        if (!res.ok || data.error) {
        ...
        if (data.done) {
          setNtMsg('Готово — усі серії мають гачок «Далі буде».')
          break
        }
        if (data.processed) {
          setNtDone(d => d + 1)
          setNtLast(`S${data.processed.season}E${data.processed.episode} · ${data.processed.title ?? ''}`)
      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 20, padding: 14, background: NAVY, borderRadius: 12, border: '1px solid rgba(208, 163, 85,0.25)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <button
              type="button"
              onClick={runNextTeaserBatch}
              disabled={ntRunning}
              style={{
                fontSize: 13, fontWeight: 700, fontFamily: FONT,
                color: '#1a1205', background: GOLD, border: 'none',
                borderRadius: 8, padding: '9px 16px',
                cursor: ntRunning ? 'default' : 'pointer', opacity: ntRunning ? 0.7 : 1,
                whiteSpace: 'nowrap',
              }}
            >
              {ntRunning ? '⏳ Генерую «Далі буде»…' : '➜ Згенерувати всі гачки «Далі буде»'}
            </button>
            <span style={{ fontSize: 12, color: '#8899bb', fontFamily: FONT }}>
              Два-три речення в кінці серії, що ведуть у наступну — блок «ДАЛІ БУДЕ» під текстом.
              Модель бачить кінець цієї серії й початок наступної. Остання серія сезону
              пропускається. Тільки для серій без гачка.
            </span>
          </div>
          {(ntRunning || ntDone > 0 || ntMsg) && (
            <div style={{ marginTop: 10, fontSize: 12, color: '#cbd5e1', fontFamily: FONT, lineHeight: 1.6 }}>
              {ntDone > 0 && <div>Згенеровано цього запуску: <b style={{ color: GOLD }}>{ntDone}</b>{ntTotal ? ` (усього серій: ${ntTotal})` : ''}</div>}
              {ntLast && <div style={{ color: '#8899bb' }}>Останній: {ntLast}</div>}
              {ntText && <div style={{ marginTop: 6, padding: 10, background: 'rgba(255,255,255,0.04)', borderRadius: 8, color: '#e7d9bf', fontStyle: 'italic' }}>{ntText}</div>}
              {ntMsg && <div style={{ color: ntMsg.startsWith('Помилка') ? '#dd8f8f' : '#9ae6b4' }}>{ntMsg}</div>}
            </div>
          )}
        </div>
