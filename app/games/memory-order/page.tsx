  ...
  return (
    <main lang="uk" style={{ minHeight: '100vh', background: `linear-gradient(180deg, ${NAVY} 0%, ${NAVY2} 50%, ${NAVY} 100%)`, padding: '32px 5% calc(120px + env(safe-area-inset-bottom, 0px))', fontFamily: "'Montserrat', sans-serif", color: '#fff' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        <p style={{ color: TEXT_SOFT, fontSize: 16.5, lineHeight: 1.6, margin: '0 0 26px' }}><b style={{ color: GOLD }}>Що це тренує.</b><br />«Робочу пам’ять» — здатність
        ...
        {/* ───────── Ігрове поле ───────── */}
        <section style={{ background: CREAM, borderRadius: 24, border: `2px solid ${GOLD_LIGHT}`, padding: '22px 18px', color: NAVY, boxShadow: '0 18px 40px rgba(0,0,0,0.28)' }}>
          {playing && (
          ...
          {phase === 'intro' && (
            <div style={{ textAlign: 'center', padding: '4px 0' }}>
              <p style={{ fontSize: 17, lineHeight: 1.5, margin: '0 0 14px', color: '#4A4234' }}>
                Дивіться, які клітинки спалахують, і повторіть той самий порядок.
                Час на повторення необмежений.
              </p>
              {best !== null && <p style={{ fontSize: 15, color: GOLD_DARK, fontWeight: 700, margin: '0 0 14px' }}>Ваш рекорд довжини: {best}</p>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
                <button onClick={() => begin(false)} style={{ ...btnPrimary, width: '100%', maxWidth: 300 }}>Почати</button>
                <button onClick={() => begin(true)} style={{ ...btnGhost, width: '100%', maxWidth: 300 }}>Спершу пробний раунд</button>
              </div>
            </div>
          )}
        ...
        <details style={detailsBox} className="bb-details">
          ...
          <div style={detailsBody}>
            <p><b>Що тренує.</b><br />Робочу пам’ять — здатність короткочасно утримувати інформацію й діяти з нею (наприклад, тримати в голові послідовність кроків). Це одна з ключових опор мислення в будь-якому віці.</p>
            <p><b>На чому ґрунтується.</b><br />У великому огляді 97 рандомізованих досліджень (2017) саме тренування робочої пам’яті дали найбільший ефект серед когнітивних вправ. Конкретні приклади: у <b>Німеччині</b> (Інститут досліджень умов праці й людських факторів, IfADo, Дортмунд, 2018) чотиримісячне тренування покращило робочу пам’ять у 141 літньої людини (середній вік 70 років); у <b>Китаї</b> (2019) рандомізоване дослідження показало покращення робочої пам’яті в літніх із легкими когнітивними порушеннями, з переносом на виконавчі функції.</p>
            <p className="bb-cream-note"><b>Чесно про користь.</b><br />Ця гра покращує саме <i>робочу пам’ять</i>, а також пов’язані з нею <i>увагу та виконавчі функції</i> (планування дій, самоконтроль) — це підтверджено дослідженнями. Сприймайте її як корисну вправу для пам’яті, а не як ліки; вона не замінює лікування чи консультацію лікаря.</p>
            <p><b>Як грати найкраще.</b><br />Користь дає регулярність — короткі заняття кілька разів на тиждень дають більше, ніж рідкі довгі. Грайте спокійно; помилка — не поразка, а сигнал, що мозок працював на межі своїх можливостей, і саме так він тренується.</p>
          </div>
        </details>
        ...
        <details style={detailsBox} className="bb-details">
          ...
          <div style={detailsBody}>
            <p><b>Кому підійде.</b><br />Будь-якому дорослому, хто хоче тренувати пам’ять і увагу — і у зрілому віці, і під час відновлення. Можна грати разом із рідними, по черзі.</p>
