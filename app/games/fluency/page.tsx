...
export default function FluencyPage() {
  ...
  // Тримаємо ref синхронним із лічильником, щоб завершення по таймеру
  // читало актуальне число (а не застигле 0 з моменту старту).
  ...
  const finish = () => {
    ...
    setBest((b) => { const nb = [...b] as [number, number, number]; if (c > nb[li]) nb[li] = c; return nb; });
    setPhase('done');
  };
      ...
      <div style={inner}>
        ...
        {phase === 'idle' && (
          <div style={{ background: CARD, borderRadius: 16, border: '1.5px solid rgba(250,199,117,0.25)', padding: '24px 18px', textAlign: 'center', boxShadow: '0 0 24px rgba(239,159,39,0.12)' }}>
            ...
            <p style={{ fontSize: 14, color: 'rgba(207,227,250,0.7)', margin: '0 0 18px' }}>Рахуємо самі — натискайте «+1» за кожне сказане вголос слово. Найцікавіше — грати по черзі з кимось: другий слухає й рахує.</p>
        ...
        <details style={detailsBox} className="bb-details">
          ...
          <div style={detailsBody}>
            ...
            <p><b>Як це влаштовано тут.</b><br />Як у справжньому тесті — одна хвилина, слова <b>вголос</b>. Але рахуєте ви себе <b>самі</b>, тож усе тримається на чесності: гра не чує й не перевіряє слів, а лише лічить ваші натискання. Тому наодинці результат вартий рівно стільки, скільки вашої відвертості, — а <b>по черзі з кимось</b> він об'єктивніший: партнер чує й контролює. Правопис не важливий, важлива швидкість пригадування.</p>
            <p className="bb-cream-note"><b>Чесні межі.</b><br />Оскільки ви рахуєте себе самі, це радше <i>вправа-самооцінка</i>, ніж точний вимір, і аж ніяк не доказ, що вона запобігає деменції. Тренування покращує насамперед саме пригадування слів; широкого перенесення на все мислення чи профілактики хвороб переконливо не доведено. Це проста корисна вправа, <b>не медичний тренажер</b> і не заміна обстеження.</p>
