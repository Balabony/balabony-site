 ...
 *
 * Правити можна і чернетку, і вже опублікований твір. Спершу тут стояли
 * лише чернетки — з остраху, що зміна заголовка потягне зміну slug. Насправді
 * slug зберігається окремо і при оновленні не перегенеровується, тому
 * посилання від правки заголовка не ламається.
 *
 * slug не чіпаємо ні за яких умов. Він міг бути виданий при імпорті і вже
 * стояти в чиємусь листі; заголовок і посилання розходяться безболісно.
 *
 ...
 *
 * Редакторської вичитки після правки немає навмисно: це власний текст автора,
 * а редакція фізично не встигає переглядати сотні творів.
 */

const EDITABLE = ['draft', 'approved', 'published']
  ...
  try {
    ...
    if (!EDITABLE.includes(String(row.status))) {
      return NextResponse.json(
        { ok: false, error: 'Цей твір зараз у роботі редакції' },
        { status: 400 },
      )
    }
