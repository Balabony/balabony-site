...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, workTitle, workType, volume, comment } = body

    if (!name || !email || !workTitle) {
    ...
    await resend.emails.send({
      ...
      replyTo: email,
      subject: `[Озвучення] Заявка від ${name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; color: #1e293b;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
            <tr><td style="padding: 8px; font-weight: bold; width: 140px;">Ім'я:</td><td style="padding: 8px;">${name}</td></tr>
            ...
            <tr><td style="padding: 8px; font-weight: bold;">Назва твору:</td><td style="padding: 8px;">${workTitle}</td></tr>
            <tr style="background: #f8fafc;"><td style="padding: 8px; font-weight: bold;">Тип:</td><td style="padding: 8px;">${workType || '—'}</td></tr>
            <tr><td style="padding: 8px; font-weight: bold;">Обсяг:</td><td style="padding: 8px;">${volume || '—'}</td></tr>
          </table>
