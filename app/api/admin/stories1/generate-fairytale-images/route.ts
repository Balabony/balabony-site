  ...
  try {
    ...
    try {
      ...
      const msg = await client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `Прочитай українську казку.

КРОК 1. Визнач ГОЛОВНОГО ГЕРОЯ: стать і вік (хлопчик / дівчинка / чоловік / жінка / конкретна тварина) та коротку зовнішність. Це найважливіше — далі цей самий герой має бути на ВСІХ ілюстраціях.

КРОК 2. Виокрем ${COUNT} ключові візуальні моменти в ХРОНОЛОГІЧНОМУ порядку (початок, розвиток, кульмінація, завершення).

ПРАВИЛА для кожного опису:
- АНГЛІЙСЬКОЮ, одне речення.
- ОБОВ'ЯЗКОВО почни з головного героя з його статтю/віком, напр. "A young boy with brown hair, ..." — у КОЖНІЙ сцені та сама стать і вік. Якщо герой хлопчик — на всіх малюнках хлопчик, НЕ дівчинка.
- Лише видиме: персонажі, природа, дія, настрій. Без тексту на зображенні, без небезпеки чи насильства.

Поверни ЛИШЕ JSON-масив із ${COUNT} рядків (самі описи сцен), без пояснень, без markdown. Назва: "${title}". Текст:\n${String(text).slice(0, 4000)}`,
        }],
      })
