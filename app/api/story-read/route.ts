...
/**
 * ТИМЧАСОВА ДІАГНОСТИКА. Прибрати після того, як облік підтвердить роботу.
 *
 * Відкрити в браузері: https://balabony.com/api/story-read
 * Робить пробний запис від імені поточного відвідувача й повертає РЕАЛЬНУ
 * помилку бази, яку POST навмисно ковтає.
 *
 * Розвилка: якщо тут ok:true, а з реального читання рядків немає — виною
 * клієнт (компонент не змонтувався). Якщо помилка — виною база чи запит.
 */
export async function GET() {
  ...
  try {
    ...
    const { data: story, error: pickErr } = await db
      .from('content')
    ...
    out.pickError = pickErr?.message ?? null
    out.story = story ?? null
    if (!story) return NextResponse.json({ ok: false, step: 'pick', ...out })
    ...
    out.insertError = error
      ...
      : null
    out.inserted = data ?? null

    const { count } = await db
      .from('article_reads')
      .select('*', { count: 'exact', head: true })
    out.rowsInTable = count ?? 0

    return NextResponse.json({ ok: !error, ...out })
  } catch (e) {
