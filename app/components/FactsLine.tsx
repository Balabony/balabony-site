...
export default function FactsLine({ initial }: { initial?: Stats } = {}) {
  ...
  // Скорочено ще раз 16.09.2026: «Для слабозорих». Це найкоротша форма, яка
  // ще називає ЛЮДИНУ, а не функцію — «великий шрифт» відхилено саме тому.
  // В ОДИН РЯДОК на телефоні воно все одно не лягає: разом із двома числами
  // це 41 знак при приблизно 30, що вміщаються. Далі скорочувати нема куди;
  // якщо рядок муляє, правильний хід — не коротше слово, а прибрати третій
  // пункт зовсім: кнопка «ДОСТУПНІСТЬ» стоїть просто над ним і каже те саме.
  // 18.09.2026: прибрано зовсім (рішення Богдана) — кнопка «ДОСТУПНІСТЬ»
  // стоїть просто над рядком і каже те саме; дубль лише подовжував рядок.
