  ...
  return (
    <>
      <style jsx global>{`
        ...
        /* ПАНЕЛЬ КЕРУВАННЯ НЕ ЛИПЛА — знайдено 17.09.2026.
           У globals.css стоїть html, body { overflow-x: hidden }. Це робить
           body контейнером прокрутки, і будь-який position: sticky всередині
           перестає працювати. Наслідок був не косметичний: читач вмикав
           високий контраст, прокручував сторінку — і кнопка «Скинути» їхала
           геть разом із панеллю, тобто вимкнути тему він уже не міг. Вада
           доступності на сторінці про доступність.

           Лікуємо через clip замість hidden: він так само обрізає
           горизонтальний виліз, але НЕ створює контейнер прокрутки, тож
           sticky оживає. Правило живе в styled-jsx цієї сторінки, отже діє
           лише поки вона відкрита — решта сайту (і sticky-шапка в Header.tsx,
           зламана з тієї ж причини) лишається як була. Те питання вирішуємо
           окремо й свідомо, а не побічним ефектом.

           important тут навмисно: селектор той самий, що в globals.css, і
           покладатися на порядок вставки стилів не можна.

           УВАГА НА МАЙБУТНЄ: у цьому файлі CSS — шаблонний літерал у зворотних
           лапках. Зворотна лапка в коментарі обриває літерал і валить білд. */
        ...
        <main id="a11y-main-content" className="a11y-main" role="main">
          ...
          <section className="a11y-inclusion" aria-labelledby="ubd-heading">
            ...
            <div>
              ...
              <p>
                Статус ВПО та УБД підтверджується за кілька секунд через «Дію» — довідок
                надсилати не треба. Людям з інвалідністю достатньо додати фото довідки чи
                посвідчення: пільга вмикається одразу, а скан видаляється після перевірки.
              </p>
